...
### RN-BACKUP-032 — Cron sempre interpretado em America/Sao_Paulo

- **Tela:** — (scheduler, `cmd/worker`, não é uma tela)
- **Status:** confirmada
- **Origem:** bug report do usuário (2026-09-17) — cron `0 3 * * *` (esperado: 3h) disparava às 00:00 (meia-noite); raiz: Go `time.Now()` no container retornava em UTC (processo não tinha `TZ` configurado nem acesso a banco de fusos horários), logo `cron.Next(time.Now())` calculava "3am UTC" = "00:00 America/Sao_Paulo"

| Condição                                                                                                                                   | Resultado                                                                                                                                                                                                                                | Fonte                                                                                                                                                                                                                                                                                                                | Status     |
| ------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| Toda expressão cron (ex: `0 3 * * *` para "todos os dias às 3h") é interpretada em America/Sao_Paulo, independente de: (a) timezone do processo Go, (b) variável `TZ` do host, (c) timezone da máquina/container em que roda | cron dispara sempre no horário correto de Sao Paulo (ex: `0 3 * * *` dispara à `06:00 UTC` = `03:00 America/Sao_Paulo`, não a `03:00 UTC` = `00:00 America/Sao_Paulo`)                                                                        | `backend/internal/scheduler/nextrun.go`: `loadBrazilTZ()` (lazy-loaded, cacheado), `NextRunTime()` converte `from` para `loc := America/Sao_Paulo` antes de chamar `cronExpr.Next(from.In(loc))` para que o cálculo de "próximo horário" sempre use o relógio brasileiro; ambos os entrypoints (`backend/cmd/api/main.go`, `backend/cmd/worker/main.go`) importam `_ "time/tzdata"` para embutir o banco IANA no binário | confirmada |
| Container/máquina roda com `ENV TZ=America/Sao_Paulo` (tanto dev `docker-compose.yml` quanto prod `docker-compose.prod.yml`)              | `time.Local` e qualquer exibição de hora sem fuso explícito também refletem Brazil time — consistência operacional além da correção de cron; `log/slog` em JSON mostra timestamps em Sao Paulo quando lidos pelo operador                   | `backend/cmd/worker/Dockerfile`, `backend/cmd/api/Dockerfile` (linhas com `ENV TZ`); `docker-compose.yml` (serviços `postgres`, `api`, `worker`); `docker-compose.prod.yml` (serviços `postgres`, `api`, `worker`, `redis`) — variável de ambiente em todos os serviços que participam do agendamento             | confirmada |
